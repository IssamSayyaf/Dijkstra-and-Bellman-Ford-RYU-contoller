...
# Reference Costs
REFERENCE_BW = 15 #Mbps
REFERENCE_delay = 3 #ms
...
class ProjectController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pathAvgTime = 0
        # Links BW
        self.linksBW = get_links_BW()
        # Links Delay
        self.linksDelay = get_links_delay()
        # Path Metric
    ...
    def get_link_cost(self, srcSwitch, dstSwitch):
        ...
        cost = 0
        if self.metric == 0:
            bw = self.linksBW[srcSwitch+'::'+dstSwitch]
            cost = 1 - (bw / REFERENCE_BW)
        elif self.metric == 1:
            delay = self.linksDelay[srcSwitch + '::' + dstSwitch]
            cost = delay / REFERENCE_delay
        elif self.metric == 3:
            cost = 1
        else:
            bw = self.linksBW[srcSwitch + '::' + dstSwitch]
            bw = 1 - (bw / REFERENCE_BW)
            delay = self.linksDelay[srcSwitch + '::' + dstSwitch]
            delay = delay / REFERENCE_delay
            cost = (0.5 * bw) + (0.5 * delay)
        return cost
    ...
    def get_bellman_path(self, src, dst):
    ...
    def find_paths(self, host_ip):
        for host in self.hosts:
            if host != host_ip:
                ...
                for path in self.get_paths(host_ip, host):
                    # print(path)
                    path_cost = 0
                    print('Path: ', path)
                ...
                best_path = self.get_bellman_path(host_ip, host)
                self.new_flow(host_ip, host, best_path)
                path_time = time.time() - start_time
                print('Path time from ', host_ip, 'to', host, ': ', path_time)
                self.pathTime.append(path_time)
        if host_ip == '192.168.1.9':
            print("Path Average Time: ", sum(self.pathTime) / len(self.pathTime))
